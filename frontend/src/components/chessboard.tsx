...
const ChessBoardView = ({ setBoardStateCallback }: ChessBoardViewProps) => {
  ...
  let [chessboard, setChessboard] = useState<ReturnType<
    typeof Chessground
  > | null>(null);
...
const ChessBoardFenPicker = ({
  setFen,
  fensToDisplay,
}: ChessBoardFenPickerProps) => {
  ...
  if (fens.length === 0) return <p>No moves are available!</p>;

  return (
    <div style={{ width: "100%" }}>
      <Button
        active={!(currentMove > 0 && fens.length > 0)}
        onClick={() => {
          if (currentMove > 0) {
      ...
      <Button
        active={!(currentMove + 1 < fens.length)}
        onClick={() => {
...
export const ChessBoardFenExplorer = ({
  ...
  let [fenCallback, setFenCallback] = useState<(fen: string) => void>(
    () => (value: string) => {
      console.log(
        `Ignored request for ${value} as callback was not registered!`
      );
    }
  );
  ...
  useEffect(() => {
    fensToDisplay.then((fens) => {
      if (fens[0].length !== 0) setFensExist(true);
    });
  }, [fensToDisplay]);
